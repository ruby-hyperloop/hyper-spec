...
describe 'hyper-spec', js: true do
  ...
  context 'expect_promise' do
    ...
    it 'will wait for the promise to resolve' do
      start = Time.now
      expect_promise do
        wait(DELAY)
      end.to eq(DELAY)
  ...
  context "TimeCop integration" do
    ...
    it "will use TimeCop frozen time" do
      ...
      expect(evaluate_ruby('Time.now.to_i')).to be_within(1).of(Time.now.to_i)
    end
    ...
    it "will use TimeCop travelling time with scaling" do
      Timecop.scale 60, Time.now-1.year do
        expect(evaluate_ruby('Time.now.to_i')).to be_within(4).of(Time.now.to_i)
        start_time = Time.now
